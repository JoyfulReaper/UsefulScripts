...
readonly REPOSITORY="s3:https://s3.us-east-005.backblazeb2.com/kgivler-backup-molasses"
...
cleanup()
{
    ...
    if [[ "$exit_code" -ne 0 ]]; then
        ...
        notify \
            "Molasses B2 maintenance FAILED" \
            "high" \
            "$message"
    fi
...
log "Starting Molasses Backblaze B2 repository maintenance."
...
log "Molasses Backblaze B2 maintenance completed successfully in $(duration)."
...
notify \
    "Molasses B2 maintenance succeeded" \
    "default" \
    "$SUCCESS_MESSAGE"
