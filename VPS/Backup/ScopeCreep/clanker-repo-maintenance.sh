...
cleanup()
{
    ...
    if [[ "$exit_code" -ne 0 ]]; then
        ...
        notify \
            "Clanker backup maintenance FAILED" \
            "high" \
            "$message"
    fi
...
exec 9>/run/lock/clanker-backup-maintenance.lock
...
notify \
    "Clanker backup maintenance succeeded" \
    "default" \
    "$SUCCESS_MESSAGE"