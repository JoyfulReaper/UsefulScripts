...
readonly REPOSITORY="s3:https://s3.us-east-005.backblazeb2.com/kgivler-backup-scopecreep"
...
cleanup()
{
    ...
    if [[ "$exit_code" -ne 0 ]]; then
        ...
        notify \
            "ScopeCreep B2 maintenance FAILED" \
            "high" \
            "$message"
    fi
...
log "Starting ScopeCreep Backblaze B2 repository maintenance."
...
log "ScopeCreep Backblaze B2 maintenance completed successfully in $(duration)."
...
notify \
    "ScopeCreep B2 maintenance succeeded" \
    "default" \
    "$SUCCESS_MESSAGE"
